...
class VidyoConnector extends Component {
  ...
  connectToConference() {
    let connectorDisconnected = connectionStatus => {
      this.setState({ joinLeaveButtonState: true, connectionStatus });
    };
    this.vidyoConnector
      .Connect({
    ...
    this.vidyoConnector
      .RegisterLocalWindowShareEventListener({
        ...
        onSelected: localWindowShare => {
          if (localWindowShare) {
            this.setState({ isSharing: true, screenShareButtonState: false });
          } else {
            this.setState({ isSharing: false, screenShareButtonState: true });
          }
        },
  ...
  screenShareButtonOnClick(screenShareButtonState) {
    ...
    let share = this.state.screenShareButtonState
      ? this.state.screenShares[Object.keys(this.state.screenShares)[0]] || null
      : null;
    ...
    return (
      <div className="vidyo-connector">
        <div
          ...
          className="renderer pluginOverlay rendererFullScreen"
        ></div>
        <EntranceForm
          host={this.state.host}
          token={this.state.token}
          resourceId={this.state.resourceId}
          displayName={this.state.displayName}
          hidden={true} //this.state.entranceFormHidden}
